...
for ii in range(len(all_cospons)):
    for jj in range(len(all_cospons[ii])):
        reps_admat[ii][all_cospons[ii][jj]] = reps_admat[ii][all_cospons[ii][jj]] + 1 #note that adjacency matrix indices are opposite of convention in physics
...
for ii in range(reps):
    ...
    tempweights = []
    for jj in range(reps):
        if reps_weights[ii][jj] > 0.0: #note that adjacency matrix indices are opposite of convention in physics
            templist.append(jj)
...
OutDegree = np.zeros(reps)
for ii in range(len(RepOutList)):
    OutDegree[ii] = np.count_nonzero(RepOutList[ii])
    
OutDegree = OutDegree/np.max(OutDegree)
